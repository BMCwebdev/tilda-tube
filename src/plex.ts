...
let cachedSectionId: string | null = null;
...
async function plexFetch(path: string, options?: RequestInit): Promise<any> {
...
async function getSectionId(): Promise<string> {
  if (cachedSectionId) return cachedSectionId;
  ...
  const movieSection = sections.find((s: any) => s.type === 'movie');
  if (movieSection) {
 ...
 */
async function findItemByPath(filePath: string, maxRetries = 8): Promise<string | null> {
  const sectionId = await getSectionId();
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    const data = await plexFetch(
      `/library/sections/${sectionId}/all?sort=addedAt:desc&limit=50`
    );
 ...
 */
async function tagCollection(ratingKey: string, collectionName: string): Promise<void> {
  const encodedCollection = encodeURIComponent(collectionName);
  await plexFetch(
    `/library/metadata/${ratingKey}?collection%5B0%5D.tag.tag=${encodedCollection}`,
    { method: 'PUT' }
  );
}
 ...
 */
export async function addToPlexCollection(filePath: string, collectionName: string): Promise<void> {
  if (!PLEX_TOKEN) {
 ...
 */
export async function backfillPlexCollections(videos: Array<{
  ...
  for (const video of videos) {
    ...
    const data = await plexFetch(
      `/library/sections/${sectionId}/all?sort=addedAt:desc&limit=500`
    );
      ...
      for (const m of media) {
        ...
        for (const part of parts) {
          if (part.file === video.file_path) {
            ...
            const alreadyTagged = collections.some((c: any) => c.tag === collectionName);
            if (alreadyTagged) {
              skipped++;
